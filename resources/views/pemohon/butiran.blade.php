                ...
                <div class="col-12">
                    ...
                    @elseif(session('failed'))
                        <div class="alert alert-danger" role="alert">
                            <button type="button" class="close" data-dis    miss="alert">×</button>
            ...
            <div class="col-12">
                ...
                <div class="card">
                    <div class="card-header">
                        {{-- <h4 class="card-title">Menu Utama</h4> --}}
                        {{-- <h4>Butiran Permohonan - {{ $maklumats->kod_permohonan ?? 'M/Draft'}}</h4> --}}
                    ...
                    <div class="card-body">
                        <dl class="row">
                          ...
                          {{-- <br> --}}
                            ...
                            <dd class="col-sm-4">
                                {{-- RM {{ number_format($maklumats->kosMohon, 2) }} --}}
                        ...
                        @if ( $maklumats->id_status != 12 )
                            ...
                            {{-- TINDAKAN --}}
                            @if ( $maklumats->id_status == 1 || $maklumats->id_status == 11 || $maklumats->id_status == 19 || $maklumats->id_status == 20 ) {{-- PENGESAH LEVEL --}}
                                {{-- @if ( $disyor ) --}}
                                <dl class="row">
                                                    ...
                                                    <tr style="font-weight: normal;">
                                                        ...
                                                        <?php
                                                            ...
                                                            switch($status){
                                                                ...
                                                                case "10": $color="#FF0000"; $status="Tidak Diluluskan"; break; //merah
                                                                //semak semula
                                                                case "11": $color="#ffc107"; $status="Semak Semula"; break; //biru
                                                                case "22": $color="#ffc107"; $status="Semak Semula"; break; //biru
                                                                //batal
                                                                case "8": $color="#CC3300"; $status="Batal"; break; //merah pekat
                                                                //sah
                                                                case "11": $color="#006400"; $status="Disahkan"; break; //hijau pekat
                                                        ...
                                                        ?>
                                                        {{-- <td> {{ ($tindakanList->UpdatedBy!='') ? \App\PPersonel::find($tindakanList->UpdatedBy)->name : '' }}</td> test --}}
                                                        ...
                                                        <td style="text-align: justify;"> {{ $disyor->Ulasan }} </td>
                                                        {{-- <td>{{ Carbon\Carbon::parse($tindakanList->CreatedAt)->format('d.m.Y ,H:i') }}</td> --}}
                                                        <td>{{ Carbon\Carbon::parse($disyor->CreatedAt)->format('d/m/Y, h:i a') }}</td>
                            ...
                            @endif
                            @if ( $maklumats->id_status == 22  || $maklumats->id_status == 21 ) {{-- PENTADBIR LEVEL --}}
                                <dl class="row">
                                    <dt class="col-sm-12" align="justify"><u><h5 style="color: black;"> Log Tindakan </h5></u> </dt>
                                    <dt class="col-sm-12" align="justify"> 
                                    <div class="table-responsive">
                      ...
                      {{-- <div class="justify-content-center"> --}}
                        <div class="btn-group float-right">
                          {{-- <button type="submit" id="hantar" class="btn btn-primary float-left btn-sm"><i
                            ...
                            <a href="{{ url('pemohon/menu/' . $user->id) }}" class="btn btn-secondary float-left btn-sm" style="color: black;">
                            {{-- <a href="{{ url('pemohon/menu/' . $personel->nokp) }}" class="btn btn-secondary float-left btn-sm"> --}}
                                 <i class="fas fa-redo-alt"></i> 
                                 | Kembali
                             </a>

                            @if( $maklumats->id_status == 1 || $maklumats->id_status == 11 || $maklumats->id_status == 22 )
                                <a href="{{ url('/pemohon/batal/'.$maklumats->idMaklumatPermohonan) }}" id="batal" class="btn btn-danger float-right btn-sm"><i
                                        class="fa fa-trash"></i> 
                                        | Batal
                                </a>
                                <a href="{{ url('/pemohon/kemaskini/'. $maklumats->idMaklumatPermohonan ) }}" id="kemaskini" class="btn btn-success float-right btn-sm"><i
                                        class="fa fa-edit"></i> 
                                        | Kemaskini
                                </a>
                            ...
                            @elseif( $maklumats->id_status == 9 )
                                ...
                                {{-- <a href="{{ url('/pemohon/cetak/' . $maklumats->idMaklumatPermohonan) }}"
                            ...
                            {{-- @elseif( $maklumats->id_status == 10 || $maklumats->id_status == 14 || $maklumats->id_status == 16 || $maklumats->id_status == 18 ) {{-- if PERMOHONAN GAGAL BOLEH REAPPLY & COPY PASTE DETAIL --}}
                            @elseif( $maklumats->id_status == 10 ) {{-- if PERMOHONAN GAGAL BOLEH REAPPLY & COPY PASTE DETAIL
                                {{-- <a href="{{ url('/pemohon/mohon_semula/'. $maklumats->idMaklumatPermohonan ) }}" class="btn btn-info float-right btn-sm" title="Mohon semula dengan maklumat yang sama."><i
                                        class="fa fa-exchange"></i> 
                                        | Mohon Semula
                                </a> --}}
                                <button type="button" name="mohon_semula" id="mohon_semula" class="btn btn-info float-right btn-sm" title="Mohon semula dengan maklumat yang sama.">
                                    <i class="fa fa-exchange"></i> | Mohon Semula
                                </button>
                            @endif
...
@section('script')
    ...
    <script type="text/javascript">
        $(function () {
            ...
            document.getElementById('mohon_semula').addEventListener('click', function() {
                Swal.fire({
                    title: 'Mohon Semula',
                    text: 'Adakah anda pasti untuk memohon semula permohonan ini?',
                    // icon: 'success', 
                    type: "warning",
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    confirmButtonText: 'Ya, Mohon Semula!',
                    allowOutsideClick: false 
                })
                .then(function (result) {
                    if (result.dismiss !== 'cancel') {
                        // Redirect to the specified URL
                        window.location.href = "{{ url('/pemohon/mohon_semula/'. $maklumats->idMaklumatPermohonan ) }}";
                    }
                });
            }); 
            //Popup Pengesahan Bile HANTAR
